...
class MeGNet(MessagePassing):
    ...
    def __init__(self,
                 ...
                 embed_size=[32, 32, 32],# edge, node ,global 
                 ):
...
class OrbitalInteractions(MessagePassing):
    """ 
    This class will handle all the interactions between atoms and will update both the edges and the nodes of the graph.
    """
    
    def __init__(self,
                 inner_skip=False,  # tels us if we read the original value or th evaluate after embedding
                 input_dim=[32, 32, 32],# edge , node global
                 output_dim=[32, 32, 32],# edge , node global
                 ):
        super().__init__(aggr="mean")
...
class PairInteractions(MessagePassing):
...
class Onsite(MessagePassing):
    ...
    def __init__(self,
        ...
        self.l2 = MeGNet(edge_shape=int(input_dim[0] / 2), node_shape=int(input_dim[1] / 2),
                         u_shape=int(input_dim[2] / 2), embed_size=[int(i / 2) for i in input_dim],
                         inner_skip=True)
        self.l3 = MeGNet(edge_shape=int(input_dim[0] / 2), node_shape=int(input_dim[1] / 2),
    ...
    def forward(self, x, edge_index, edge_attr, state, batch, bond_batch):
        ...
        x, edge_attr, state = self.l3(x, edge_index, edge_attr, state, batch, bond_batch)
        return   x,  edge_attr, state
...
class Ofsite(MessagePassing):
    ...
    def __init__(self,
        ...
        self.l2 = MeGNet(edge_shape=int(input_dim[0] / 2), node_shape=int(input_dim[1] / 2),
                         u_shape=int(input_dim[2] / 2), embed_size=[int(i/2) for i in input_dim ],
                         inner_skip=True)
        self.l3 = MeGNet(edge_shape=int(input_dim[0]/2), node_shape=int(input_dim[1]/2), u_shape=int(input_dim[2]/2), embed_size=output_dim,
                         inner_skip=True)
    def forward(self, x, edge_index, edge_attr, state, batch, bond_batch):
        """
        :param x: Node proprieties
