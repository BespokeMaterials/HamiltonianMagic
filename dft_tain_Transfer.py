...
def hop_on_difference(pred, targets):
    ...
    targets_0 = targets[0][:, 0]/10  # .reshape([pred[0].shape[0] * 2])
    pred_1 = pred[1][:, 0]  # .reshape([pred[1].shape[0] * 2])
    targets_1 = targets[1][:, 0]/10   # .reshape([pred[1].shape[0] * 2])
    # print("pred", pred)
...
## Save spot  ##
def save_spot(exp_name, spot_nr, model, data,scale=1):
    # Create directory
    ...
    for ko, inputs in enumerate(data):
        ...
        target_mat_i=target_mat_i/scale
        target_mat_r = target_mat_r / scale
...
def main(exp_name, train_data_path, test_data_path):
    ...
    save_spot(model=model,
              exp_name=exp_name,
              spot_nr=5,
              data=test_dataloader,scale=10)


if __name__ == "__main__":
    exp_name = "Classic_mixt_Transfer_mini"
